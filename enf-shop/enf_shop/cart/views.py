...
from .forms import AddToCardForm, UpdateCartItemForm
import json
...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request, slug):
        ...
        if size_id:
            ...
            product_size = product.product_sizes.filter(stock__gte=0).first()
            if not product_size:
                return JsonResponse({
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                return JsonResponse({
                'error': f'Cannot add {product_size.stock} items. Only  {product_size.stock - existing_item.quantity} more available.'
            }, status=400) 
...
class UpdateCartItemView(CartMixin, View):
    ...
    def post(self, request, item_id):
        ...
        context = {
            'cart': cart,
            'cart_items': cart.items.select_related( # Check this
                'product',
                'product_size__size',
            ).order_by('-added_at')
        }
